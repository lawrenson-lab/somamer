...
library(ggrepel)
#read the data
data<-readxl::read_excel("227_0001_01_UTHSA_Report.xlsx",sheet = "Proteins")
...
data%>%
  ...
  apply(2,function(x) #apply to every column (represented by x) the function in line 24
...
## Check samples in columns
```{r}
temp<-data%>%
  select(SeqID,Counts_BEME004_Control_Control_NA:Counts_Test00005_Endometriosis_NA_NA)%>%
  column_to_rownames("SeqID")#rownames need to be unique
summary(temp)%>%view()#to see the range of the expression values per sample
...
temp<-temp%>%
  ...
  mutate(type=str_split_i(sample,pattern = "_",i = 3))#create a new column named type by
  #splitting sample names at every "_" an keeping the 3rd value 
temp%>%ggplot(aes(x=PC1,y=PC2,col=type))+#to color by the new type column
  geom_point()+#for scatterplot
  geom_text_repel(data=temp%>%slice_min(PC2,n = 3),aes(x=PC1,y=PC2,label=sample))+
  geom_text_repel(data=temp%>%slice_min(PC1,n = 3),aes(x=PC1,y=PC2,label=sample))
#spot the outlier
#you can check if any of the PCs separates the groups
temp%>%
  select(type,PC1:PC2)%>%
  pivot_longer(-1,names_to = "comp",values_to = "variate")%>%
  group_by(comp)%>%#for every component
  wilcox_test(variate~type)%>%#see if coordinates change between groups
  view()
