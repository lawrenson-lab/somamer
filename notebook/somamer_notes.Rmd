---
title: "Tips to start. Cut/paste/modify what you need"
---
aims:
...
#maker the data columns order match metadata
mat<-mat[,meta$Sample]
...
res<-res%>%rownames_to_column("feature")
#nothing is significant
summary(res$adj.P.Val)
#volcano plot
res%>%ggplot(aes(x=logFC,y=-log10(P.Value)))+geom_point()+
  #add labels to the most different genes
  geom_text_repel(data=res%>%filter(abs(logFC)>.75),
                  aes(x=logFC,y=-log10(P.Value),label=feature))
```
# Does enrichment make sense with this type of data?
```{r}
library(clusterProfiler)
res<-res[order(match(res$feature,rownames(mat))),]
res$Uniprot<-data$Uniprot

#what is the best way to deal with duplicates?
i<-res$uniprot[duplicated(res$uniprot)]
res%>%filter(uniprot=="P02765")
#               feature       logFC  AveExpr          t   P.Value adj.P.Val         B uniprot
#1   a2.HS.Glycoprotein -1.39555513 8.059832 -1.5094355 0.1351575 0.9998487 -4.401547  P02765
#2 a2.HS.Glycoprotein.1  0.03507062 5.013690  0.4782737 0.6337658 0.9998487 -4.898613  P02765
rank<-res%>%group_by(uniprot)%>%slice_max(abs(logFC))%>%
  arrange(desc(logFC))%>%select(uniprot,logFC)%>%deframe()
gseares<-gseGO(geneList = rank,keyType = "UNIPROT",ont="BP",OrgDb = 'org.Hs.eg.db')
```
